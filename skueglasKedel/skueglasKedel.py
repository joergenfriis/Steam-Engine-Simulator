# skueglasKedel.py
#
# Modul, der styrer den Arduino, der regulerer vaeskestanden i skueglasset med kedelvand.
#
# Slave adresse 0x37
#
...
def set(niveau):
    ...
    for _ in range (30):
        try:
            bus.write_i2c_block_data(0x37,0,[niveau]) # send niveau til arduinoen
...
def plus1():
    ...
    for _ in range (30):
        try:
            bus.write_i2c_block_data(0x37,0,[110]) # send 110 til arduinoen
            # if we get here, we succeeded, so break out of the loop
            Success = True
            break
        except:
...
def minus1():
    ...
    for _ in range (30):
        try:
            bus.write_i2c_block_data(0x37,0,[111]) # send 111 til arduinoen
            # if we get here, we succeeded, so break out of the loop
            Success = True
            break
        except:
...
def reset():
    ...
    for _ in range (30):
        try:
            bus.write_i2c_block_data(0x37,0,[112]) # send 112 til arduinoen
            # if we get here, we succeeded, so break out of the loop
            Success = True
            break
        except:
